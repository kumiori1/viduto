...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const setMobileView = () => {
      if (checkMobile()) {
        ...
        const existingStyle = document.getElementById('mobile-no-scroll');
    ...
    const initialize = async () => {
      ...
      try {
        ...
        if (urlParams.get('success') === 'true') {
          ...
          const refreshCreditsAfterPurchase = async () => {
            let attempts = 0;
            ...
            const intervalId = setInterval(async () => {
              ...
              if (attempts >= maxAttempts) {
                ...
                console.log('Stopped aggressive credit refresh');
              }
            }, 2000);
          };
          
          refreshCreditsAfterPurchase();
  ...
  return (
    <div className={`h-screen md:h-screen h-[calc(var(--vh,1vh)*100)] flex overflow-hidden ${darkMode ? 'bg-gray-900' : 'bg-white'}`}>
      {sidebarOpen && (
        <div
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <div className={`md:hidden p-4 flex items-center justify-between flex-shrink-0 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} border-b`}>
            <button
